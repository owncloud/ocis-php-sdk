...
class EducationSchoolTest extends TestCase
{
    ...
    public function testGetEducationSchool(array $data): void
    {
        ...
        $user = $ocis->getEducationSchools($educationApi);
        $this->assertCount(1, $user);
        $this->assertContainsOnlyInstancesOf(EducationSchool::class, $user);
        $this->assertSame("id", $user[0]->getId());
        $this->assertSame("demo", $user[0]->getDisplayName());
        $this->assertSame("1", $user[0]->getNumber());
        $this->assertInstanceOf(DateTime::class, $user[0]->getTerminationDate());
        $this->assertSame('2020-12-12', $user[0]->getTerminationDate()->format('Y-m-d'));
    }
